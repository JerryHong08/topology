...
topology decouples structure from representation, allowing humans to see and agents to reason over the same system.
...
## Background
    ...
    the agents receives the input and read the codes related.
                            |    ▲ 
                            |    |
                            |    |
                            ▼    |
    the human reads the agents outputs text and test the functionality.

    is there a new interface that let us adapts to this change? 
    what is the best practice to leverage the agents to help us do the coding work while we can have better observability and feedback adjusted loop?
